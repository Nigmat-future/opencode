...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const selectedKey = args.provider
      ? options.find(
          (option) => option.value === args.provider || database[option.value]?.name?.toLowerCase() === args.provider?.toLowerCase(),
        )?.value
      : yield* promptValue(
          yield* Prompt.autocomplete({
            message: "Select provider",
    ...
    if (!selectedKey) return yield* fail(`Unknown configured provider "${args.provider}"`)
    const credential = credentials.find((item) => item.key === selectedKey)
